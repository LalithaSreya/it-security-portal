-- ==========================================
-- IT Security Portal Database Fix Script (v3 - Table-Backed RLS)
-- ==========================================
-- Run this script in your Supabase SQL Editor (https://supabase.com/dashboard/project/_/sql)
-- to create missing tables, and set up robust, database-backed RLS policies.
-- This version reads user roles directly from the employees table to avoid JWT sync issues.
...
-- Recreate Task Policies (using database-backed role checks)
create policy "Allow Managers full access to tasks" on public.tasks
  for all using (
    (select role from public.employees where id = auth.uid()) = 'Manager'
  );
...
-- Recreate Report Policies (using database-backed role checks)
...
-- 3. FIX EMPLOYEES TABLE RLS POLICIES (RECURSION-FREE & TABLE-BACKED)
...
drop policy if exists "Allow Managers and Admins to write employees" on public.employees;
...
-- Policy A: Allow authenticated users to read employees list (recursion-free SELECT check)
create policy "Allow authenticated users to read employees" on public.employees
  for select using (auth.role() = 'authenticated');

-- Policy B: Allow individual employees to update their own profile details (recursion-free UPDATE check)
create policy "Allow employees to update own record" on public.employees
  for update using (auth.uid() = id) with check (auth.uid() = id);

-- Policy C: Allow Managers and Admins to insert, update, and delete employees
-- (Using specific write targets querying employees table, which is recursion-free since SELECT has a simple check)
create policy "Allow Managers and Admins to insert employees" on public.employees
  for insert with check (
    (select role from public.employees where id = auth.uid()) in ('Admin', 'Manager')
  );

create policy "Allow Managers and Admins to update employees" on public.employees
  for update using (
    (select role from public.employees where id = auth.uid()) in ('Admin', 'Manager')
  ) with check (
    (select role from public.employees where id = auth.uid()) in ('Admin', 'Manager')
...
create policy "Allow Managers and Admins to delete employees" on public.employees
  for delete using (
    (select role from public.employees where id = auth.uid()) in ('Admin', 'Manager')
  );
